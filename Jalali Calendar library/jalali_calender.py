    ...
    def Moon_in_Scorpio_Calender(your_year=None, your_month=None, Gregorian_year_1=None, Gregorian_year_2=None):
        ...
        def Moon_in_Scorpio_Choose_day(days, months, years):
            ...
            if col_days >= 7.60 and col_days <= 8.98:
                return 'Today the moon is in Scorpio.', round(col_days,
                                                              2), day_shamsi, month_shamsi, day_name_shamsi, month_name_shamsi, years_sh
            else:
                return 'The moon is not in Scorpio today.', round(col_days,
                                                                  2), day_shamsi, month_shamsi, day_name_shamsi, month_name_shamsi, years_sh
        ...
        def Mooon_is(months=None, years=None, mons=None):
            ...
            for i in range(1, mons):
                st, co, sh, mo, day_name, month_name, years_sh = Moon_in_Scorpio_Choose_day(i, months, years)
        ...
        if Gregorian_year_1 == None or Gregorian_year_2 == None:
            Gregorian_year_1 = datetime.date.today().year
            Gregorian_year_2 = Gregorian_year_1 + 1
        elif Gregorian_year_1 != None or Gregorian_year_2 != None:
            Gregorian_year_1 = Gregorian_year_1
            Gregorian_year_2 = Gregorian_year_2

        Gregorian_years = [Gregorian_year_1, Gregorian_year_2]

        ad_years = []
        for i in range(1, 13):
            t = Mooon_is(i, Gregorian_years[0], g_days_in_month[i - 1])
            ad_years.append(t)
            if i == 12:
                for j in range(1, 5):
                    t = Mooon_is(j, Gregorian_years[1], g_days_in_month[j - 1])
                    ad_years.append(t)
